        ...
        <div className="flex flex-row min-h-screen">
            ...
            <nav className="h-screen max-sm:hidden sm:w-fit lg:w-[257px] bg-white border-r-[1px] border-r-[rgb(0,0,0,0.09)] transition-all duration-300">
                ...
                <ul className="flex flex-col flex-1">
                        <li className="py-2 px-4"><SidebarItem icon={LayoutDashboard} gap={1} text="ภาพรวม" path="/dashboard" currentPath={currentPath} className="shadow-sm text-nowrap"/></li>
                        <li className="py-2 px-4"><SidebarItem icon={PlusCircle} gap={1} text="เพิ่มบัญชีร้านค้า" path="/createshop" currentPath={currentPath} className="shadow-sm text-nowrap"/></li>
                        <li className="py-2 px-4"><SidebarItem icon={Store} gap={1} text="ร้านค้าทั้งหมด" path="/shops" currentPath={currentPath} className="shadow-sm text-nowrap"/></li>
                        <li className="py-2 px-4"><SidebarItem icon={Users} gap={1} text="ลูกค้าทั้งหมด" path="/users" currentPath={currentPath} className="shadow-sm text-nowrap"/></li>
                </ul>
            </nav>
